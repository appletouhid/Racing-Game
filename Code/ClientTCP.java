 ...
 * @author abaltouhid
 */
public class ClientTCP extends JFrame implements Runnable, KeyListener, ActionListener, MouseListener {
    ...
    private boolean rightrotate = false, gameover = false, paint = false, clientAcceptance = false, scoreShow = true, animeCtrl = false;

    //Constractor that connects with the server and create the GUI
    ...
    public ClientTCP(String serverName, int serverPort) {
        ...
        try {
            //Socket is used to connect with the server
            socket = new Socket(serverName, serverPort);
            System.out.println("Connected: " + socket);

            String parts[] = socket.toString().split(",");
            String part3 = parts[2];
            StringBuffer sb = new StringBuffer(part3);
            sb.deleteCharAt(sb.length() - 1);
            String client[] = sb.toString().split("=");
            ID = client[1];
            System.out.println("\nID: " + ID + "\n");
    ...
    //run the client after getting acceptance from the server
    public void run() {
        while (thread != null) {
            try {
                //CarGame cg = new CarGame("Car Game");
                if (clientAcceptance) {
                    streamOut.writeUTF("Client Added");
                    streamOut.flush();
                    clientAcceptance = false;
                }
                if (gameover && scoreShow) {
    ...
    //starts the client with the healp of thread
    ...
    //draw the road, tree, cars and apply the animation
...
//			}
            ...
            //score 
            g.setColor(Color.gray);
            g.fillRect(10, 35, 220, 50);
            g.setColor(Color.DARK_GRAY);
            g.fillRect(15, 40, 210, 40);
            g.setColor(Color.gray);
            ...
            g.setFont(new Font("Serif", Font.BOLD, 50));
            g.setColor(Color.yellow);
            g.drawString("Game Start", 210, 270);
            g.setColor(Color.white);
            g.setFont(new Font("Arial", Font.BOLD, 30));
            g.drawString("Press Space to Start", 190, 340);
            if (!paint) {
                repaint();
                paint = true;
            }
    ...
    //all the key pressed event are controlled from here
    @Override
    ...
    public static void main(String args[]) {
        ...
        client = new ClientTCP("localhost", 2000);
    }
}
